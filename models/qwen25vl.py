...
class Qwen25VL(BaseModel):
    def __init__(self, 
                 path="/home/chenxiang.101/workspace/checkpoints/Qwen2.5-VL-7B-Instruct", 
                 device='cuda:0', 
                 dtype=torch.bfloat16, 
                 name='Qwen2.5-VL-7B'):
        super(Qwen25VL, self).__init__(path, device, name)
    ...
    def batch_chat(self, prompts, images=None, max_new_tokens=1024):
        ...
        for i in range(N):
            ...
            text_infos = self.processor.apply_chat_template(
                messages, tokenize=False, add_generation_prompt=True
            )
            image_infos = None
            if image is not None:
                image_infos, _ = process_vision_info(messages)
            text_inputs.append(text_infos)
...
if __name__ == '__main__':
    instance = Qwen25VL()
    image = "/home/chenxiang.101/workspace/images/x2.png"
    prompt = "这张图像中绿色车牌上的字是什么？"
    # response = instance.chat(prompt, image=image)
    # print(response)   

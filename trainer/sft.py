...
from .dataset import LanguageSupervisedDataset, EOUAugmentedDataset, AISHELLAudioDataset
...
from .utils import language_data_collator, safe_save_model_for_hf_trainer, get_parameter_number
...
def make_language_supervised_data_module(
    data_args,
    processor,
    ...
    raw_data_list = load_json_datas(data_args.data_path)
    rank0_print(f"All data num: {len(raw_data_list)}， data_type: {data_args.data_type}")
    
    if data_args.data_type == 'eou':
        train_dataset = EOUAugmentedDataset(
            raw_data_list,
            processor,
    ...
    elif data_args.data_type == 'aishell':
        train_dataset = AISHELLAudioDataset(
            raw_data_list,
            processor,
            max_length=max_length,
        ...
        train_dataset = LanguageSupervisedDataset(
            raw_data_list,
            processor,
            max_length=max_length,
            llm_type=llm_type
    ...
    return dict(
        ...
        data_collator=partial(data_collator, padding_value=padding_value, max_length=max_length, collate_labels=True),
    )
    

def load_from_pretrained(model_path, device_map, dtype=torch.bfloat16, attn_implementation='eager'):
...
def train():
    ...
    model, tokenizer = load_from_pretrained(
        model_args.model_name_or_path, device_map, dtype=compute_dtype, 
